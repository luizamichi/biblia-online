...
class CapituloDAO {
	...
	public static function update(Capitulo $capitulo): bool {
		...
		foreach($capitulo->versiculos as $versiculo) {
			$consulta .= "UPDATE `versiculos` SET `versiculo_versao_id` = :versiculo_versao_id" . $versiculo->chave . ", `versiculo_livro_id` = :versiculo_livro_id" . $versiculo->chave . ", `versiculo_capitulo` = :versiculo_capitulo" . $versiculo->chave . ", `versiculo_numero` = :versiculo_numero" . $versiculo->chave . ", `versiculo_texto` = :versiculo_texto" . $versiculo->chave . " WHERE `versiculo_id` = :versiculo_id" . $versiculo->chave . ";";
			$parametros[] = [
